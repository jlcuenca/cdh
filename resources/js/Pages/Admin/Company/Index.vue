<template>
    <admin-layout>
        <template v-slot:head>
            <h1>Empresas</h1>
            <Link :href="route('manager.companies.create')" class="rounded-full h-12 w-12 shadow bg-blue-500 text-white hover:bg-blue-600 transition duration-100 flex items-center justify-center content-center">
        ...
        <template v-slot:content>
            <table-component>
                <template v-slot:thead>
                    ...
                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" width="115"></th>
                </template>
                <template v-slot:tbody>
                  <tr v-for="record in records.data">
                    <td class="px-6 py-4 whitespace-nowrap  font-medium text-gray-900">
                        <Link :href="route('manager.companies.show', {id: record.id})" class="text-blue-500 hover:text-maroon-600">
                          {{ record.name }}
                        </Link>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center font-medium">
                      <Link :href="route('manager.companies.edit', {id: record.id})" class="text-blue-500 hover:text-maroon-600">
                        Editar
                      </Link>
                    </td>
                  </tr>
